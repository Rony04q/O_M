...
              .insert({ 
                ...
                total_price: total, // Use 'total_price' from your schema
                payment_mode: 'Card', 
                payment_status: 'paid',
                delivery_status: 'pending' // Use 'delivery_status' instead of 'status'
              })
              // --- END OF FIX ---
              .select('id').single();
...
        {/* Back Button */}
        <Button variant="ghost" onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6 transition-colors px-0">
...
        _            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold transition-all ${
...
Indicates the model should return audio.                    </div>
                     {/* Payment Review */}
                     <div className="border-b pb-4">
                d       <h3 className="font-semibold text-gray-900 mb-3">Payment Method</h3>
                        <div className="flex items-center gap-2">
                           <CreditCard className="w-5 h-5 text-gray-400" />
                           <p className="text-gray-600">Card ending in {formData.cardNumber.slice(-4)}</p>
Tuning mode is unspecified.                     </div>
                     </div>
                     {/* Items Review */}
Indicates the model should return text                  <div>
                        <h3 className="font-semibold text-gray-900 mb-3">Order Items</h3>
                        <div className="space-y-3">
                           {cartItems.map(item => (
                              <div key={item.id} className="flex gap-4">
                                 <img src={item.image} alt={item.name} className="w-16 h-16 object-cover rounded-lg"/>
Indicates the model should return images.                            <div className="flex-1">
                                   <h4 className="font-medium text-gray-900">{item.name}</h4>
Indicates the model should return audio.                              <p className="text-sm text-gray-500">Quantity: {item.quantity}</p>
                                 </div>
                                 <p className="font-semibold text-gray-900">
Indicates the model should return text                                {new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(item.price * item.quantity)}
                                 </p>
                Hi, I'm Dora—someone who chases every new turn in AI, and lately, local models have been my go-to. Last month, I was mid-drafting a tech review when my internet cut out. My cloud AI tool froze, but I'd just installed Ollama the night before. I clicked it open, typed my question, and got a response in seconds—right on my laptop. That's when it hit me: local AI isn't just a “nice option”—it's a game-changer for how we use AI.              </div>
                           ))}
s                    </div>
                     </div>
                 </div>
...
                {step > 1 && (<Button type="button" variant="outline" onClick={() => setStep(step - 1)} disabled={isProcessing}>Back</Button>)}
                <Button type="submit" disabled={isProcessing} className="flex-1 bg-emerald-600 text-white hover:bg-emerald-700">
...
Note: The rest of your JSX form is perfect, so I've omitted it for brevity. It does not need to change.                <div className="flex justify-between text-sm"><span className="text-gray-600">Tax ({ (taxRate * 100).toFixed(0) }%)</span><span className="font-medium text-gray-900">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(tax)}</span></div>
